...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < tam; i++) // TEM QUE TER PIXELS DE ORIGEM NA SAIDA
    {
        // PEGA PIXELS DE INDICE RANDOMICO E UNICO DA SAIDA E COMPARA COM DESEJADA
        int indice = rand() % ultimo;
        int r = array[indice];
        array[indice] = array[ultimo];
        ultimo--;
        // int r = i;

        // SE USAR i JUNTO COM int n = (rand() % (tam - i)) + i, FICA COM PIXELS OTIMOS NO INICIO E O RESTO NO FIM
        RGBpixel desejada = pic[DESEJ].pixels[r];
        int menor_diff = 100000;
        int index = 0;
        // ACHA INDICE DE PIXEL MAIS PAREDCIDO COM O DA IMAGEM DESEJADA
        for (int j = 0; j < 100; j++)
        {
            int real = (rand() % (ultimo + 1)) - 1;
            int n = array[real];
            // int n = (rand() % (tam - i)) + i;// PIXELS OTIMOS NO INICIO
            //int n = rand() % tam;

            RGBpixel saida = pic[SAIDA].pixels[n];
            //DIFERENCA ENTRE PIXEL DE SAIDA EM N E PIXEL DESEJADO EM R
            int diff1 = abs(saida.r - desejada.r) + abs(saida.g - desejada.g) + abs(saida.b - desejada.b);
            
            //DIFERENCA ENTRE PIXEL DE SAIDA EM N E PIXEL DESEJADO EM N
            RGBpixel desejado = pic[DESEJ].pixels[n];
            int diff2 = abs(saida.r - desejado.r) + abs(saida.g - desejado.g) + abs(saida.b - desejado.b);
            
            if (diff1 < menor_diff && diff1 < diff2)
            {
                menor_diff = diff1;
                index = n; // GUARDA INDICE DE MELHOR PIXEL
            }

        }
        // TROCA MELHOR PIXEL DA IMAGEM DE SAIDA
        RGBpixel paux = pic[SAIDA].pixels[r];
        pic[SAIDA].pixels[r] = pic[SAIDA].pixels[index];
        pic[SAIDA].pixels[index] = paux;
        // COLOCA OS MELHORES PIXELS NO INICIO E O RESTO NO FINAL DA IMAGEM :|
        // OU
        // SE IMAGEM TEM MUITO UMA COR, ESPALHA COR POR TODA A IMAGEM :|
    }
